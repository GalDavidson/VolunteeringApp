    ...
    class VolunteeringAPIProxy
    {
        ...
        public async Task<bool> UpdateEvent(DailyEvent de)
        {
            try
            {
                ...
                string jsonObject = JsonSerializer.Serialize<DailyEvent>(de, options);
                StringContent content = new StringContent(jsonObject, Encoding.UTF8, "application/json");

                HttpResponseMessage response = await this.client.PostAsync($"{this.baseUri}/UpdateEvent", content);
                if (response.IsSuccessStatusCode)
                {
                    jsonObject = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> UpdateRate(VolunteersInEvent vol)
        {
            try
            {
                JsonSerializerOptions options = new JsonSerializerOptions
                ...
                string jsonObject = JsonSerializer.Serialize<VolunteersInEvent>(vol, options);
                StringContent content = new StringContent(jsonObject, Encoding.UTF8, "application/json");

                HttpResponseMessage response = await this.client.PostAsync($"{this.baseUri}/UpdateRating", content);
                if (response.IsSuccessStatusCode)
                {
                    jsonObject = await response.Content.ReadAsStringAsync();
                    bool ret = JsonSerializer.Deserialize<bool>(jsonObject, options);
        ...
        public async Task<bool> RemoveVolFromEvent(DailyEvent de)
        {
            try
            {
                JsonSerializerOptions options = new JsonSerializerOptions
                ...
                StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
                HttpResponseMessage response = await this.client.PostAsync($"{this.baseUri}/RemoveVolFromEvent", content);
                if (response.IsSuccessStatusCode)
                {
                    string jsonContent = await response.Content.ReadAsStringAsync();
                    bool success = JsonSerializer.Deserialize<bool>(jsonContent, options);
        ...
        public async Task<DailyEvent> NewEvent(DailyEvent d)
        {
            try
            {
                ...
                string jsonObject = JsonSerializer.Serialize<DailyEvent>(d, options);
                StringContent content = new StringContent(jsonObject, Encoding.UTF8, "application/json");

                HttpResponseMessage response = await this.client.PostAsync($"{this.baseUri}/CreateNewEvent", content);
                if (response.IsSuccessStatusCode)
                {

                    string str = await response.Content.ReadAsStringAsync();

                    DailyEvent ev = JsonSerializer.Deserialize<DailyEvent>(str, options);
                    return ev;
                }
                else
                {
